...
def main(args):
    ...
    try:
        data = google_crop_hints(data)
        data = flattenDict(data)
        return {'payload': {'response': data}}
    except:
        return {'payload': {'response': ['Exception occurred']}}

def google_crop_hints(json_obj):
    temp = []
    rec = json_obj['responses']
    for a1 in rec:
        rec1 =  a1['cropHintsAnnotation']['cropHints']
        for a2 in rec1:
            result = {}
            result['boundingPoly'] = a2['boundingPoly']['vertices']
            result['importanceFraction'] = a2['importanceFraction']
            result['confidence'] = a2['confidence']
            temp.append(result)
    return temp
