...
function main(params) {
    var from = params.payload.translate_from || 'en';
    var to = params.payload.translate_to || 'fr';
    var url = 'https://gateway.watsonplatform.net/language-translator/api';

    var texts = getTextsToTranslate(params.payload.input_text);
    var promise = new Promise(function(resolve, reject) {
        doTranslateTexts(texts, from, to, params.properties.username_ibm, params.properties.password_ibm, url, function (error, translatedTexts) {
            if (error) {reject({payload:{error}});}
            else {
                try{
                var response = setTranslatedTexts(texts,{translatedTexts:translatedTexts});
                function ibm_lang_translate(data){
                    temp = [];
                    res = data
                    for(var i = 0 , l= res.length;i<l;i++){
                        result = {};
                        result['translated_text'] = res[i];
                        temp.push(result);
                    }
                }

                ibm_lang_translate(response);
                response = temp;
...
/* Return all the string values in an object. This will recursively traverse the object. */
...
/* Set the string values in an object to the translated values. This will recursively traverse the object. */
...
function doTranslateTexts(texts, from, to, username, password, url, next) {
    ...
    language_translation.translate(options, function (err, response) {
        if (err) {
            ...
            for (var e in response.translations) {translated.push(removeDiacritics(response.translations[e].translation));}
        }
        if (next) {next(null, translated);}
    });
}

/*Return a string with diacritics removed. From http://web.archive.org/web/20120918093154/http://lehelk.com/2011/05/06/script-to-remove-diacritics */
